...
| Codex 护眼提醒周期 | `20min` | 仅 Codex mode 启用，到点显示气泡。 |
| Codex 护眼气泡时长 | `8000ms` | 气泡自动消失，不要求用户确认。 |
| 拖拽阈值 | `4px` | 超过后进入拖拽态。 |
...
| Codex 护眼提醒 | Codex mode 中每 `20min` 触发 | 不切换 mood | `8000ms` | 仅显示气泡 `Bibo~ 主人，看看远处 20 秒吧，拉姆陪你。`；拖拽中 / `sleep` / `walk` 时跳过；托盘菜单可暂停或恢复。 |
...
| R10 | Codex 护眼提醒气泡优先于 Codex 状态气泡显示；护眼气泡自动消失后恢复原状态气泡。 |
